...
void PrepareShutdown()
{
    ...
    util::ThreadRename("polarisnetwork-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the POL and zPOL money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 1997, 2997));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all POLARISNETWORK specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-polstake=<n>", strprintf(_("Enable or disable staking functionality for POL inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zpolstake=<n>", strprintf(_("Enable or disable staking functionality for zPOL inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 1998, 2998));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The POLARISNETWORK Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("polarisnetwork-loadblk");
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. POLARISNETWORK Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. POLARISNETWORK Core is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //POLARISNETWORK specific: zerocoin and spork DB's
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zpol if already reindexed
                    RecalculatePOLSupply(1, fReindexZerocoin);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
            if (prev_version <= FEATURE_PRE_POLARISNETWORK && pwalletMain->IsLocked()) {
        ...
        if (fFirstRun) {
            if (!fLegacyWallet) {
                ...
                pwalletMain->SetMaxVersion(FEATURE_PRE_POLARISNETWORK);
            }
        ...
        zwalletMain = new CzPOLWallet(pwalletMain);
        ...
        if (!zwalletMain->GetMasterSeed().IsNull()) {
            //Inititalize zPOLWallet
            uiInterface.InitMessage(_("Syncing zPOL wallet..."));

            //Load zerocoin mint hashes to memory
            pwalletMain->zpolTracker->Init();

...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 POL.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZpolSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZpol(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt)
{
...
static void NotifyzPOLReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzPOLReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzPOLReset.connect(boost::bind(NotifyzPOLReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzPOLReset.disconnect(boost::bind(NotifyzPOLReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (address.IsStakingAddress())
        return error("Invalid POLARISNETWORK address, cold staking address");
    CKeyID keyID;
...
{
    if (!address.IsValid())
        return error("Invalid POLARISNETWORK address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from POLARISNETWORK address");
...
{
    setMints.clear();
    setMints = pwalletMain->zpolTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}

...
#include "qt/polarisnetwork/dashboardwidget.h"
#include "qt/polarisnetwork/forms/ui_dashboardwidget.h"
#include "qt/polarisnetwork/sendconfirmdialog.h"
#include "qt/polarisnetwork/txrow.h"
...
DashboardWidget::DashboardWidget(POLARISNETWORKGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquarepol, "square-chart-pol");
    setCssProperty(ui->labelSquarezpol, "square-chart-zpol");
    setCssProperty(ui->labelpol, "text-chart-pol");
    setCssProperty(ui->labelZpol, "text-chart-zpol");
    ...
    setCssProperty(ui->labelAmountpol, "text-stake-pol-disable");
    setCssProperty(ui->labelAmountZpol, "text-stake-zpol-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &POLARISNETWORKGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &POLARISNETWORKGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &POLARISNETWORKGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool ispol = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZPOL;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair POL, zPOL
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            pol = (pair.first != 0) ? pair.first / 100000000 : 0;
            zpol = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalpol += pair.first;
            chartData->totalZpol += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuespol.append(pol);
        chartData->valueszpol.append(zpol);

        int max = std::max(pol, zpol);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    // init sets
    set0 = new QBarSet("POL");
    set1 = new QBarSet("zPOL");
    ...
    set0->append(chartData->valuespol);
    set1->append(chartData->valueszpol);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalpol > 0 || chartData->totalZpol > 0) {
        setCssProperty(ui->labelAmountpol, "text-stake-pol");
        setCssProperty(ui->labelAmountZpol, "text-stake-zpol");
    } else {
        setCssProperty(ui->labelAmountpol, "text-stake-pol-disable");
        setCssProperty(ui->labelAmountZpol, "text-stake-zpol-disable");
    }
    forceUpdateStyle({ui->labelAmountpol, ui->labelAmountZpol});
    ui->labelAmountpol->setText(GUIUtil::formatBalance(chartData->totalpol, nDisplayUnit));
    ui->labelAmountZpol->setText(GUIUtil::formatBalance(chartData->totalZpol, nDisplayUnit, true));

...
#include "qt/polarisnetwork/forms/ui_snackbar.h"
#include "qt/polarisnetwork/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(POLARISNETWORKGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &POLARISNETWORKGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {

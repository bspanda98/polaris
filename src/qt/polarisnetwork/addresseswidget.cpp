...
#include "qt/polarisnetwork/forms/ui_addresseswidget.h"
#include "qt/polarisnetwork/addresslabelrow.h"
#include "qt/polarisnetwork/addnewaddressdialog.h"
#include "qt/polarisnetwork/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(POLARISNETWORKGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CBitcoinAddress polAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(polAdd.Get(), label.toUtf8().constData(),
                polAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {

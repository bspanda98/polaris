...
#include "qt/polarisnetwork/topbar.h"
#include "qt/polarisnetwork/forms/ui_topbar.h"
#include "qt/polarisnetwork/lockunlock.h"
#include "qt/polarisnetwork/qtutils.h"
#include "qt/polarisnetwork/receivedialog.h"
#include "qt/polarisnetwork/loadingdialog.h"
...
TopBar::TopBar(POLARISNETWORKGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezpol, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzpol, ui->labelTitleImmaturezpol};
    ...
    setCssProperty({ui->labelAmountToppol, ui->labelAmountTopzpol}, "amount-small-topbar");
    setCssProperty({ui->labelAmountpol, ui->labelAvailablezpol}, "amount-topbar");
    setCssProperty({ui->labelPendingpol, ui->labelPendingzpol, ui->labelImmaturepol, ui->labelImmaturezpol}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("POLARISNETWORK")
    updateDisplayUnit();
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // POL Total
    CAmount polAvailableBalance = balance;
    // zPOL Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalpol = GUIUtil::formatBalance(polAvailableBalance, nDisplayUnit);
    QString totalzpol = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);

    // POL
    // Top
    ui->labelAmountToppol->setText(totalpol);
    // Expanded
    ui->labelAmountpol->setText(totalpol);
    ui->labelPendingpol->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmaturepol->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));

    // Update display state and/or values for zPOL balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of zPOL label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopzpol->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzpol->setText(totalzpol);
    // Expanded
    ui->labelAvailablezpol->setText(totalzpol);
    ui->labelPendingzpol->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturezpol->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}

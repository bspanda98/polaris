...
#include "qt/polarisnetwork/send.h"
#include "qt/polarisnetwork/forms/ui_send.h"
#include "qt/polarisnetwork/addnewcontactdialog.h"
#include "qt/polarisnetwork/qtutils.h"
#include "qt/polarisnetwork/sendchangeaddressdialog.h"
#include "qt/polarisnetwork/optionbutton.h"
#include "qt/polarisnetwork/sendconfirmdialog.h"
#include "qt/polarisnetwork/myaddressrow.h"
#include "qt/polarisnetwork/guitransactionsutils.h"
...
SendWidget::SendWidget(POLARISNETWORKGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-pol");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onPOLSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onPOLSelected(false);});
...
void SendWidget::refreshView()
{
    ...
    ui->pushButtonSave->setText(isChecked ? tr("Send POL") : tr("Send zPOL"));
...
void SendWidget::refreshAmounts()
{
    ...
    bool isZpol = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZpol));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend POL (not zPOL)
    const bool isZpol = ui->pushRight->isChecked();
    const bool isCControl = CoinControlDialog::coinControl->HasSelected();
    const bool hasDel = cachedDelegatedBalance > 0;

    const bool showCheckBox = !isZpol && !isCControl && hasDel;
    ...
    if (showCheckBox)
        ui->checkBoxDelegations->setToolTip(
                tr("Possibly spend coins delegated for cold-staking (currently available: %1").arg(
                        GUIUtil::formatBalance(cachedDelegatedBalance, nDisplayUnit, isZpol))
        );
}
...
void SendWidget::onSendClicked()
{
    ...
    bool sendpol = ui->pushLeft->isChecked();
    ...
    if ((sendpol) ? send(recipients) : sendZpol(recipients)) {
...
bool SendWidget::sendZpol(QList<SendCoinsRecipient> recipients)
{
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zPOL is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zPOL selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZpolControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZpolControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZpol(
        ...
        inform(tr("zPOL transaction sent!"));
        ZpolControlDialog::setSelectedMints.clear();
        clearAll(false);
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZPOL_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zPOL require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().GetConsensus().ZC_MaxSpendsPerTx; // Maximum possible spends for one zPOL transaction
        ...
        Q_EMIT message("zPOL transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked()
{
    if (isPOL) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any POL to select."));
        }
    } else {
        if (walletModel->getZerocoinBalance() > 0) {
            ZpolControlDialog *zpolControl = new ZpolControlDialog(this);
            zpolControl->setModel(walletModel);
            zpolControl->exec();
            ui->btnCoinControl->setActive(!ZpolControlDialog::setSelectedMints.empty());
            zpolControl->deleteLater();
        } else {
            inform(tr("You don't have any zPOL in your balance to select."));
        }
    }
}
...
void SendWidget::onPOLSelected(bool _isPOL)
{
    isPOL = _isPOL;
    setCssProperty(coinIcon, _isPOL ? "coin-icon-pol" : "coin-icon-zpol");
    refreshView();
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CBitcoinAddress polAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(polAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(polAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {

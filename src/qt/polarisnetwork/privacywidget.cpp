...
#include "qt/polarisnetwork/privacywidget.h"
#include "qt/polarisnetwork/forms/ui_privacywidget.h"
...
PrivacyWidget::PrivacyWidget(POLARISNETWORKGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    // Only Convert to POL enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzPOL->setTitleClassAndText("btn-title-grey", tr("Total 0 zPOL"));
    ui->btnTotalzPOL->setSubTitleClassAndText("text-subtitle", tr("Show denominations of zPOL owned."));
    ui->btnTotalzPOL->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", tr("Coin Control"));
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", tr("Select POL outputs to mint into zPOL."));
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", tr("Reset Spent zPOL"));
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", tr("Reset zerocoin database."));

    connect(ui->btnTotalzPOL, &OptionButton::clicked, this, &PrivacyWidget::onTotalZpolClicked);
...
void PrivacyWidget::onMintSelected(bool isMint)
{
    QString btnText;
    if (isMint) {
        btnText = tr("Mint zPOL");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of POL to mint into zPOL"));
    } else {
        btnText = tr("Convert back to POL");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zPOL to convert back into POL"));
    }
...
void PrivacyWidget::onTotalZpolClicked()
{
    bool isVisible = ui->layoutDenom->isVisible();
    if (!isVisible) {
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzPOL->setRightIconClass("btn-dropdown", true);
    } else {
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzPOL->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zPOL is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zPOL").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zPOL minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value)
{
    ...
    if (!walletModel->convertBackZpol(
            value,
            selectedMints,
            receipt
    )) {
        ...
        // Spend succeed
        inform(tr("zPOL converted back to POL"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply()
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPOL </b>";
    ...
    ui->btnTotalzPOL->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}

...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZPOLModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zpolTracker->HasMintTx(hash))) {
            //zPOL stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZPOL;
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime, wtx.GetTotalSize());
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zpol;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by POLARISNETWORK Address
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address)) {
                    ...
                    if (wtx.HasZerocoinMintOutputs())
                        continue;
                    // Sent to POLARISNETWORK Address
...
bool IsZPOLType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZPOL:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zpol:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZPOL ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZPOL);
}

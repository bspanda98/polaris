...
bool CpolStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CpolStake from zerocoin spend");
...
bool CpolStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CpolStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CpolStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CpolStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CpolStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CpolStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CpolStake::GetUniqueness() const
{
    //The unique identifier for a POL stake is the outpoint
...
CBlockIndex* CpolStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CpolStake::ContextCheck(int nHeight, uint32_t nTime)
{

...
#include "zpol/zpolmodule.h"
#include "zpol/zpolwallet.h"
#include "zpol/zpoltracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_POLARISNETWORK = 61000, // inherited version..
...
// Possible states for zPOL send
enum ZerocoinSpendStatus {
    ZPOL_SPEND_OKAY = 0,                            // No error
    ZPOL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPOL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPOL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPOL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPOL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPOL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPOL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPOL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPOL_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZPOL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPOL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPOL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPOL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPOL_SPENT_USED_ZPOL = 14,                      // Coin has already been spend
    ZPOL_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZPOL_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 POL output and keys which can be used for the Masternode
    ...
    bool CreateZPOLOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zPOL wallet
    CzPOLWallet* zwalletMain{nullptr};
    std::unique_ptr<CzPOLTracker> zpolTracker{nullptr};
    void setZWallet(CzPOLWallet* zwallet);
    CzPOLWallet* getZWallet();
    ...
    // zPOL reset
    boost::signals2::signal<void()> NotifyzPOLReset;

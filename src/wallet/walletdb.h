...
public:
    ...
    bool WriteZPOLSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZPOLSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZPOLSeed_deprecated(uint256& seed);
    bool EraseZPOLSeed();
    bool EraseZPOLSeed_deprecated();

    bool WriteZPOLCount(const uint32_t& nCount);
    bool ReadZPOLCount(uint32_t& nCount);

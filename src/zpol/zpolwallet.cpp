...
CzPOLWallet::CzPOLWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zpol seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPOLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPOLSeed_deprecated()) {
                    LogPrintf("%s: Updated zPOL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpol seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpol wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPOLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPOLCount(nCountLastUsed);
    else if (!walletdb.ReadZPOLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPOLWallet::Lock()
{
    seedMaster.SetNull();
}

void CzPOLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPOLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPOL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPOLWallet::LoadMintPoolFromDB()
{
...
void CzPOLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPOLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPOLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zpolTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPOLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPOL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpolTracker which also adds to database
    wallet->zpolTracker->Add(dMint, true);
...
void CzPOLWallet::SeedToZPOL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzPOLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPOLWallet::GenerateDeterministicZPOL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPOLWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPOLWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPOLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

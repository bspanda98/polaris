...
bool CLegacyZpolStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZpolStake from non zc-spend");
    ...
    *this = CLegacyZpolStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zpol stake origin", __func__);

    // All good
    return true;
}

CLegacyZpolStake::CLegacyZpolStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZpolStake::GetIndexFrom()
{
...
CAmount CLegacyZpolStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZpolStake::GetUniqueness() const
{
...
bool CLegacyZpolStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (nHeight < consensus.height_start_ZC_SerialsV2 || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zPOL stake block: height %d outside range", __func__, nHeight);

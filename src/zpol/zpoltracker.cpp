// Copyright (c) 2018-2020 The POLARISNETWORK developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zpol/deterministicmint.h>
#include "zpoltracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
#include "zpol/zpolwallet.h"


CzPOLTracker::CzPOLTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzPOLTracker::~CzPOLTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPOLTracker::Init()
{
...
bool CzPOLTracker::Archive(CMintMeta& meta)
{
...
bool CzPOLTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPOLTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPOLTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPOLTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzPOLTracker::GetSerialHashes()
{
...
CAmount CzPOLTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPOLTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPOLTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPOLTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPOLTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPOLTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPOLTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPOLTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPOLTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPOLTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzPOLTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPOLWallet* zPOLWallet)
{
    bool iszPOLWalletInitialized = (nullptr != zPOLWallet);
    CMintMeta meta;
    ...
    if (!iszPOLWalletInitialized)
        zPOLWallet = new CzPOLWallet(wallet);
    meta.isSeedCorrect = zPOLWallet->CheckSeed(dMint);
    if (!iszPOLWalletInitialized)
        delete zPOLWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPOLTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPOLTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPOLTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPOLTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPOLTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPOLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzpol from DB\n", __func__, listDeterministicDB.size());
    }

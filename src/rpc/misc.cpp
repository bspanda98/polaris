...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total polarisnetwork balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zPOLsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zPOL denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zPOL denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zPOL denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zPOL denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zPOL denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zPOL denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zPOL denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zPOL denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zPOL denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in polarisnetwork/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in polarisnetwork/kb\n"
    ...
    UniValue zpolObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zpolObj.push_back(Pair(std::to_string(denom), ValueFromAmount(0)));
        else
            zpolObj.push_back(Pair(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zpolObj.push_back(Pair("total", ValueFromAmount(GetZerocoinSupply())));
    obj.push_back(Pair("zPOLsupply", zpolObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"polarisnetworkaddress\"\n"
            "\nReturn information about the given polarisnetwork address.\n"

            "\nArguments:\n"
            "1. \"polarisnetworkaddress\"     (string, required) The polarisnetwork address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"polarisnetworkaddress\",    (string) The polarisnetwork address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for POLARISNETWORK cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: POLARISNETWORK address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are polarisnetwork addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) polarisnetwork address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"polarisnetworkaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"polarisnetworkaddress\"  (string, required) The polarisnetwork address to use for the signature.\n"

...
- [Development guidelines](#development-guidelines)
    - [General POLARISNETWORK Core](#general-polarisnetwork-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
POLARISNETWORK Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
POLARISNETWORK Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/polarisnetwork-project/polarisnetwork/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_polarisnetwork
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_polarisnetwork --log_level=test_suite
$ valgrind -v --leak-check=full src/polarisnetworkd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_polarisnetwork.coverage/index.html`.
```
...
- BitcoinMiner : Generates POLs (if wallet is enabled).
...
be sent upstream but bugfixes may also be prudent to PR against POLARISNETWORK Core so that they can be integrated
...
file descriptors it will cause problems with POLARISNETWORK's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:POLARISNETWORK-Project/POLARISNETWORK.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `polarisnetwork-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `polarisnetwork-cli` and the GUI debug console use this table to determine how to

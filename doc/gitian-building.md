...
*Setup instructions for a gitian build of POLARISNETWORK Core using a VM or physical system.*
...
to the POLARISNETWORK GitHub release page.
...
- [Building POLARISNETWORK Core](#building-polarisnetwork-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/polarisnetwork-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/polarisnetwork-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/polarisnetwork-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/polarisnetwork-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/polarisnetwork-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/polarisnetwork-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/polarisnetwork-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@polarisnetwork.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/polarisnetwork-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/polarisnetwork-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/polarisnetwork-osx-*-build.assert
```

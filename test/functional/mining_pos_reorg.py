...
from test_framework.test_framework import polarisnetworkTestFramework
...
class ReorgStakeTest(polarisnetworkTestFramework):
    ...
    def check_money_supply(self, expected_pol, expected_zpol):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected POL and zPOL supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_pol))
            for denom in node['zPOLsupply']:
                assert_equal(node['zPOLsupply'][denom], DecimalAmt(expected_zpol[denom]))
    ...
    def run_test(self):
        ...
        # Check POL and zPOL supply at the beginning
        # ------------------------------------------
        # zPOL supply: 2 coins for each denomination
        expected_zpol_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # POL supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zpol_supply)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zpol",
        ...
        # Verify that POL and zPOL supplies were properly updated after the spends and reorgs
        self.log.info("Check POL and zPOL supply...")
        ...
        expected_zpol_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zpol_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zpol_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zpol_supply)

...
class ZerocoinSpendTest(polarisnetworkTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zpol_bal, pol_bal):
            zpol_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zpol_bal)
            pol_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], pol_bal)
            return zpol_bal, pol_bal
        ...
        zpol_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zpol_balance, 6666)
        ...
        zpol_balance, balance = check_balances(denom_2, zpol_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zpol_balance, balance = check_balances(denom_3, zpol_balance, balance)
        ...
        _, _ = check_balances(denom_1, zpol_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
